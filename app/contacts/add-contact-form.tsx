...
const contactSchema = z.object({
  ...
  createWorkspaceAccess: z.boolean().default(false),
});
...
export function AddContactForm({ onAddContact, setDialogOpen, activeTab }: AddContactFormProps) {
  ...
  const form = useForm<ContactFormValues>({
    ...
    defaultValues: {
      ...
      email: "",
      address: "",
      createWorkspaceAccess: false,
    },
  });

  // Watch email field to enable/disable workspace access checkbox
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 pt-4">
        ...
        <FormField control={form.control} name="createWorkspaceAccess" render={({ field }) => (
          <FormItem className="flex flex-row items-start space-x-3 space-y-0 rounded-md border p-4">
            ...
            <div className="space-y-1 leading-none">
              ...
              <FormDescription>
                {hasEmail
                  ? "Generate login credentials and create a dedicated workspace for this contact"
                  : "Email address required to create workspace access"
                }
              </FormDescription>
            </div>
          </FormItem>
