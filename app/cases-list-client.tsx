...
export default function CasesListClient({ 
  ...
  const handleAddCase = async (data: Omit<CaseFormValues, 'caseNumber'> & { workspaceId?: string }) => {
    try {
      // For workspace users, always use their assigned workspace
      let workspaceIdToUse = data.workspaceId;
      if (currentUser?.role === 'workspace_user' && currentUser?.workspaceId) {
        workspaceIdToUse = currentUser.workspaceId;
      } else if (activeWorkspace?.name === 'Main Workspace') {
        workspaceIdToUse = undefined;
      } else if (activeWorkspace?.id) {
        workspaceIdToUse = activeWorkspace.id;
      }
  ...
  const filteredAndSortedCases = hydratedCases
    .filter(c => {
      ...
      if (currentUser?.role === 'workspace_user') {
        // Workspace users can ONLY see cases in their assigned workspace
        const userWorkspaceId = currentUser.workspaceId;
        if (!userWorkspaceId) return false;
        
        // Check if case belongs to the user's workspace
